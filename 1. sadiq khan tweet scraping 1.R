...
library(igraph)
library(ggraph)
khan_tweets <- search_tweets(q = "@sadiqkhan", n = 20000, lang = "en", include_rts = FALSE)

khan_tweets %>% group_by(source)%>% 
...
khan_tweets$stripped_text <- gsub("http.*","", khan_tweets$text)
khan_tweets$stripped_text <- gsub("https.*","", khan_tweets$stripped_text)

# Second, remove punctuation, convert to lowercase, add id for each tweet!
khan_tweets_clean <- khan_tweets %>%
  dplyr::select(stripped_text) %>%
  unnest_tokens(word, stripped_text)

# Third, remove stop words from your list of words
cleaned_tweet_words <- khan_tweets_clean %>%
...
cleaned_tweet_words %>%
  ...
  labs(y = "Count",
       ...
       subtitle = "analysis of 17,773 tweets between 20-30 March 2018", caption = "\nSource: Data collected from Twitter's REST API via rtweet") + theme(legend.position = "")

# remove punctuation, convert to lowercase, add identity for each tweet!
khan_tweets_paired_words <- khan_tweets %>%
  dplyr::select(stripped_text) %>%
  unnest_tokens(paired_words, stripped_text, token = "ngrams", n = 2)

khan_tweets_paired_words %>%
  count(paired_words, sort = TRUE)

khan_tweets_separated_words <- khan_tweets_paired_words %>%
  separate(paired_words, c("word1", "word2"), sep = " ")

khan_tweets_filtered <- khan_tweets_separated_words %>%
  filter(!word1 %in% stop_words$word) %>%
  filter(!word2 %in% stop_words$word)

# new bigram counts:
khan_words_counts <- khan_tweets_filtered %>%
...
khan_words_counts %>%
  ...
  labs(title = "Word Network: Tweets sent to @sadiqkan",
       subtitle = "analysis of 17,773 tweets between 14-23 March 2018",
