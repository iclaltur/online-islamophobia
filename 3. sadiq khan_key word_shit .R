...
library(igraph)
library(ggraph)
khan_shit_tweets <- search_tweets(q = "@sadiqkhan AND shit", n = 20000, lang = "en", include_rts = FALSE)

khan_shit_tweets %>% group_by(source)%>% 
...
khan_shit_tweets$stripped_text <- gsub("http.*","", khan_shit_tweets$text)
khan_shit_tweets$stripped_text <- gsub("https.*","", khan_shit_tweets$stripped_text)

# Second, remove punctuation, convert to lowercase, add id for each tweet!
khan_shit_tweets_clean <- khan_shit_tweets %>%
  dplyr::select(stripped_text) %>%
  unnest_tokens(word, stripped_text)

# Third, remove stop words from your list of words
cleaned_tweet_words <- khan_shit_tweets_clean %>%
